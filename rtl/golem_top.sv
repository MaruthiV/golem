// The whole chip, autonomous: a control FSM drives generation (feed token -> wait ->
// stream token out UART -> feed it back), so no host orchestration is needed. This is
// what runs on the Tang Nano. (sdram_model here is the sim stand-in; on the board it is
// replaced by the real GW2AR SDRAM controller on the same command port, and the SDRAM
// pins become top-level ports.)
module golem_top #(
  ...
  // golem + arbiter + SDRAM (the board memory subsystem)
  ...
  // control FSM: generate a story, stream each token id as 2 bytes (hi then lo)
  ...
  always_ff @(posedge clk) begin
    ...
    if (rst) begin cst <= C_INIT; done <= 1'b0; end
      ...
      C_NEXT:   begin
        pos <= pos + 8'd1;
        if (pos == MAX_TOKENS - 8'd1 || cur == START_TOK) cst <= C_DONE;
        else cst <= C_START;
      end
      C_DONE:   done <= 1'b1;
      default:  cst <= C_INIT;
