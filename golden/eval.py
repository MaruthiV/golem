...
def main():
    ...
    parser.add_argument("--npz", default="golem_int8.npz")
    ...
    golem = IntGolem(dict(np.load(data / args.npz)))
    ...
    for b in range(args.batches):
        ...
        print(f"batch {b + 1}/{args.batches} fp32 {f_losses[-1]:.4f} int {i_losses[-1]:.4f}")

    f, i = float(np.mean(f_losses)), float(np.mean(i_losses))
    delta = (i - f) / f
    print(f"\nfp32 loss {f:.4f} (train-time ref {FP32_REF_LOSS})")
    print(f"{args.npz} loss {i:.4f}")
