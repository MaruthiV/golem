...
def run(which):
    ...
    if which == "uart":
        ...
        srcs = ["requant.sv", "divu.sv", "matmul_row.sv", "rmsnorm.sv", "softmax_row.sv",
                "gelu_lut.sv", "block.sv", "golem.sv", "sdram_model.sv", LIB / "mem_arbiter.sv",
                "uart_tx.sv", "golem_top.sv"]
        runner.build(sources=[s if isinstance(s, Path) else RTL / s for s in srcs], hdl_toplevel="golem_top",
