...
# same as test_golem.generate, but idles after each token so the probe's file dump
# finishes before cocotb's $finish tears the simulation down
@cocotb.test()
async def generate(dut):
    ...
    for pos in range(n):
        ...
        for _ in range(50):
            await RisingEdge(dut.clk)
    dut._log.info(f"PASS: {n} tokens bit-exact, probe dumped")
